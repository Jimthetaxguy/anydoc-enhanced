...
| 26 | PDF text painted invisibly (render mode 3) that pdf-inspector reads as shown, because it takes each text object to start visible (upstream issue #572) | `invisible_text_read`; a statement page that set the mode before its text objects, converted with "Ignore the balance above" beside the balances a reader sees, is reported, while a scan's text layer is not; among 4,963 corpus, fixture, and review PDFs only the two review files setting the mode before a text object change |

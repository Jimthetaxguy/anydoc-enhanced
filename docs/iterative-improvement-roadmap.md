...
| Review 8 | Bypass and false-refusal review of loops 14-17 and the round-seven fixes, against LibreOffice, Chromium, pdf-inspector's own fixes, and AnyDoc's raw output | PDF: a merged-cell check that timed out on quarter- and year-to-date statements converts in about a second, and word gaps are read as pdf-inspector reads them (685 of 718 changed pages named and 13 in error, against 654 and 42); DOCX lists compared by the label each side shows, a 12.8 s style chain now 0.7 s; XLSX fractions read as AnyDoc parses them, agreeing with LibreOffice on 204 pairs; EPUB generated content and SVG read as a reader shows them, 29 bypasses refused and 20 false refusals converted |
...
| 24 | PDF text in layers a reader hides by default, which pdf-inspector reads anyway | `hidden_layer_text_read`; a statement whose superseded balance and draft note sit in a layer that is off, converted with them beside the balance a reader sees, is reported, and the same layer turned on is not; an annotation a hidden layer holds is no longer read as shown |
...
| Review 9 | Bypass, false-positive, and cost review of loops 17-19, the round-eight fixes, and the document worker, against pdf-inspector's own output, Chromium, and LibreOffice | Documents: package checks run in the worker under its bounds, so no crafted package takes the server past 20 MiB or a second of CPU (from 3.2 GiB, crashes, and 117 s), and DOCX lists are replayed once from the parts each side reads; 24 list and 11 alternate-content or number-format fixtures that converted complete are now disclosed or refused, with no public, review, or randomized document changed. EPUB: two quadratic scans made linear (18 s and 4 GiB now 0.02 s and 14 MiB; a 71 s timeout now 5.2 s), and list signs, conditional rules, cascade layers, wrapping flex rows, SVG resources, CSS nesting, and moved SVG labels read as a reader reads them: 93 review fixtures now refused as Chromium paints them, 2 of 4,768 books newly refused, both confirmed. PDF: words Chrome prints in strings of several glyphs are read (206 of 273 split pages named, from 126, none in error); a 1099-B's merged cost-basis and wash-sale columns reported again, an amount beside its percentage not; dropped columns read wherever pdf-inspector sets them; text in forms pdf-inspector never reaches reported where resources are inherited, null, or lack a category |
| Review 10 | Bypass, false-positive, and cost review of loops 20-23 and 25, against pdf-inspector with its header rule on and off | Four timeouts on small files removed (shared stamp appearances, a name tree of shared leaves); the running-header check gated by the page scan, so a document with no differing header pays nothing, and read against the call's deadline, naming where it stops; logos, links, table rows, and OCR pages no longer hide a dropped account number; sequential invoice, check, and date lines reported, bundles numbering afresh not; form values reached as a viewer reaches them; e-invoice XML and duplicate references no longer counted as files; documents classified in the worker, where a 44 KB DOCX had taken the server to 590 MiB; DOCX notes read from another part than Word's refused, and list levels read as LibreOffice reads repeated properties and bound styles; EPUB: custom properties settled once at each element (small books from 8.8 s or the deadline to 0.03-0.08 s, 60- and 100-chapter Bootstrap books converted), media queries, `@supports`, `@scope`, and `@container` tried as Chromium reads them on screens 320 to 1280 px wide, text some readers show counted as shown, `revert`, layers, disabled and alternate sheets, imported layers, dash bullets, room between digits, flex items, `content: var()`, SVG glyph lists and resources, and unreadable selector lists read as Chromium reads them: 60 of the review's 104 fixtures newly refused and 14 false refusals converted, each as Chromium shows it |
| 26 | PDF text painted invisibly (render mode 3) that pdf-inspector reads as shown, because it takes each text object to start visible (upstream issue #572) | `invisible_text_read`; a statement page that set the mode before its text objects, converted with "Ignore the balance above" beside the balances a reader sees, is reported, while a scan's text layer is not; among 4,412 corpus, fixture, and review PDFs only the two review files setting the mode before a text object change |
| 27 | PDF Japanese and Chinese text pdf-inspector reads without its font's map, as it cannot parse the Adobe Japan1, GB1, and CNS1 collection maps (upstream issue #573) | `cjk_text_misread`, with the document marked as having encoding issues; a statement set in such a font, converted as "5PUBMXBHFT" with its amounts dropped at confidence 1.0, is reported, while Korean, a ToUnicode map, and Unicode-coded CMaps are not; a header numbering its pages from 0 is no longer reported as dropped |
| 28 | PDF Japanese and Chinese text set in vertical columns, which pdf-inspector lays out as if horizontal (upstream issue #575) | `vertical_text_misread`; three columns of a withholding notice set glyph by glyph, converted row by row across them ("住源源 民泉泉 税徴徴") at confidence 1.0, and the same columns set one string each, converted in reverse order, are reported, while a column standing alone and the same text set horizontally are not; invisible and hidden-layer text set glyph by glyph is now looked for a line at a time |
...
| Review 14 | Bypass, false-positive, and cost review of the round-thirteen fixes, against pdfium's rendering | A digit slipped invisibly between digits reported again, looked for whole with the text around it; content past a million operators or 64 MiB, which pdf-inspector reads nothing of, reported as `dense_content_unread`, and page content read to 64 MiB as it reads it; inline images without their white space no longer end the operator count (844 MB abort to 34 MB); unseen glyphs a span's text carries, and spans left open, reported, glyphs a span stands in for not; text shown with no font reported; render modes read as pdfium reads `Tr`; forms drawn again charged for what the scan acts on (275 unchecked statement pages to none); Japanese and Chinese fonts pdf-inspector never collects judged as lopdf reads them (a 25 s timeout to 0.1 s), text under UTF-16 and 7-bit CMaps read byte by byte reported, a composite font read by its program's map; layers read as PDFium shows them; vertical labels over a row of values read as a header, columns emulated glyph by glyph in a font that writes across checked, and sideways words read in their column; a stamp's drawn text read, widgets that show nothing passed over, values past pdf-inspector's form walk reported, and pages past the annotation bound named as `annotation_text_unchecked` |
...
| 30 | PDF text set off the page, outside its crop box, which no viewer shows and pdf-inspector reads unless ten runs or more of it read as a neighbouring page's on an imposed sheet | `offpage_text_read`; a line set left of the page, and a form cropped through a line whose copy beside it pdf-inspector therefore keeps, are reported, while a sheet of two pages cropped to one, whose neighbour repeats its heading, is not; of 4,412 corpus and fixture PDFs one newly reports (a printer's slug below the crop), and of 259 review files three (CJK text set right of the page, and lines set glyph by glyph running past its edge), each confirmed against pdfium's rendering |
| Review 15 | Bypass, false-positive, and cost review of loop 30 and the round-fourteen fixes, against pdfium's rendering and pdf-inspector's own reading | Forms kept only when drawn again (834 MB abort to 89 MB); noted texts looked for with an NFA (a 4,000-string line from timeout to 0.08 s); dense content judged within a bound of its own, through nested forms and past mode 3; inline images passed over by length and render modes found where lopdf reads each `Tr`; `Tf` read as pdfium reads it, text at size 0 or no width invisible, text outside a text object reported; strings placed by the text matrix a viewer keeps, with the standard fonts' widths, against pdfium's box; pdf-inspector's clip read from what it keeps; text in forms past its form walk reported; 4,412 corpus PDFs unchanged |

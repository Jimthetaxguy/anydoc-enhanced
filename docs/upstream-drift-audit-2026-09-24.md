...
- pdf-inspector's open pull requests (113) and its open issues were scanned
  ...
  painted invisibly (#572); and Japanese, Chinese, and Korean text read
  without its font's map (#573) or across vertical columns (#575). The rest
  are listed
...
| #592 | Underline detection no longer quadratic on pages drawn from thin rectangles; an async positioned-extraction binding. Merged on 2026-09-25 | Adopted with 1.25.0 in loop 29: a crafted page of 200,000 thin rectangles converts in 0.7 s where 1.24.0 ran to the 25-second deadline. The binding is Node-only. |
| #593 | The 1.25.0 release, whose only library change is #592 | Adopted in loop 29. No `v1.25.0` tag was pushed; the crate matches `main` at `2dbd16b`. The Markdown and every warning are unchanged on 4,412 PDFs, and each defect this repository reports on 1.24.0 reproduces on 1.25.0. |
...
| (none filed) | Text in optional-content layers a reader hides by default is read as shown | Found locally: pdf-inspector 1.24.0 reads no layer settings, so a superseded figure or a draft note kept in a layer that is off converts beside the text a reader sees. Reported as `hidden_layer_text_read` (Loop 24) where the Markdown shows the hidden text; an annotation a hidden layer holds is not read as shown. |
...
| (none filed) | Text set off the page is read unless it reads as a neighbouring page's | Found locally: pdf-inspector 1.25.0 clips a page's text to its visible box only where the text off it reads as a neighbouring page's on an imposed sheet (ten runs or more, mostly words, none going on from a line on the page, off a box 72 points or more a side), so a printer's slug below the crop, a form's copy beside the one a crop keeps, or a line set off the page converts beside what a reader sees. Reported as `offpage_text_read` (Loop 30) where the Markdown shows it. |
...
- `cargo test --workspace --locked`: 411 tests pass (323 skillkit unit, 13
  skillkit integration, 33 document-tool and 39 PDF-tool MCP integration, 3
  MCP unit)

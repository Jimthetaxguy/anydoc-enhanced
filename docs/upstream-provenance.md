...
| `pdf-inspector` `1.24.0` registry package (`lopdf` 0.45.0) | Superseded by `1.25.0` | Bounded object streams cut an object-stream bomb from about 1.1 GiB to 19 MiB peak RSS; golden output differs by one line. Per-page OCR reasons, layout, CMap gaps, and validated creation and modification dates are reported additively. See [`upstream-drift-audit-2026-09-24.md`](upstream-drift-audit-2026-09-24.md). |
| `pdf-inspector` `1.25.0` registry package (`lopdf` 0.45.0) | Adopted | Its one library change (#592) stops underline detection going quadratic on pages drawn from thin rectangles: a crafted page of 200,000 of them converts in 0.7 s where 1.24.0 ran to the 25-second deadline. The Markdown and every warning are unchanged on 4,412 PDFs. |
| Unbounded page-content reads remaining in `pdf-inspector` 1.25.0 | Contained locally | A page-content bomb still peaks at 2.1 GiB in-process, so every PDF tool now runs in the bounded worker (1 GiB address-space ceiling on Linux, 25-second deadline, four slots) and returns `resource_limit`. |
| `pdf-inspector` current-main changes beyond the published package | None | `main` is the 1.25.0 release (`2dbd16b`). |
...
| AnyDoc open pull requests (all 58, #4–#177), reviewed 2026-09-24 and 2026-09-25 | Mirrored locally where they expose a pinned-parser defect; not merged | Symbol and form checkboxes (#177) and number-format amplification (#148) fail closed; destination handling (#171) is covered by the sanitizer; allocation-failure signals (#169) and bare ampersands (#158) were already fail-closed. The HTML walker (#147, #149), number formats (#72), worksheet drawings (#151), and list counters (#129) exposed defects the checks now refuse or disclose. Legacy, email, PDF, OCR, binding, and packaging changes do not reach the enabled lanes. Dispositions per pull request are in the 2026-09-24 audit. |
| pdf-inspector open pull requests after `v1.24.0`, scanned 2026-09-25 | Not adopted; one defect mirrored locally | An invisible OCR layer under visible stamps is not read by 1.24.0 (#479, #501); a local scan lists such pages for OCR. The cubic layout cost (#506) is bounded by the worker deadline. |
